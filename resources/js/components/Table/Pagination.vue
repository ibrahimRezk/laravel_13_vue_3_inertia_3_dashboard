...
//       { paginationNumber }  = storeToRefs(useGeneral);
// })
...
watch(
  ...
  () => {
    console.log(paginationNumber.value);
    if (props.withAxios) {
      emit("callAxiosUrl", {
        url: urlLink.value.url,
        paginationNumber: paginationNumber.value,
      });
    } else {
...
<template>
  <nav aria-label="Page navigation" class="grid w-full grid-cols-12">
    <div class="px-5 col-span-2">
      ...
      >
        ...
        <SelectContent class="min-w-20">
          <SelectGroup>
            <SelectLabel class="flex items-center justify-center px-0"
              >select</SelectLabel
            >
            <SelectItem
              v-for="n in [10, 20, 30, 40, 50, 100]"
              :key="n"
              :value="n.toString()"
            >
              {{ n }}
    ...
    <div class="col-span-8 flex justify-center py-0.5">
      <ul
        class="flex rounded-full border border-zinc-500 dark:border-zinc-400/70 overflow-hidden"
      >
        <li
          v-for="link in links"
          :key="link.label"
            ...
            :class="{
              'text-zinc-900 dark:text-gray-500':
                ...
                (link.label.includes('Previous') ||
                  ...
                  link.label.includes('...')),
              'bg-zinc-900  text-gray-100  font-semibold dark:font-normal dark:text-white dark:bg-zinc-500':
                link.active,
              'font-bold text-gray-900 dark:font-normal dark:text-gray-400': !isDisabled(
                link
              ),
              'border-x border-zinc-400/30 ':
